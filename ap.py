...
def sent_anly_prediction():
    if(request.method=='POST'):
        ...
        x_test=[[word_to_id[word] if (word in word_to_id and word_to_id[word]<=20000)else 0 for word in words]]#converting words to numeric id
        x_test=sequence.pad_sequences(x_test,maxlen=500)
        vector=np.array([x_test.flatten])#flatting id to a single string of word ids
